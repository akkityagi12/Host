...
from ..version import akki_version as ult_ver
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                ftext = "**Akki Client Error:** `Forward this to` @tgr_hacker_group\n\n"
                ftext += "**Py-Akki Version:** `" + str(pyver)
                ftext += "`\n**Akki-Ub Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START AKKI CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Akki Client Error:** `Forward this to` @tgr_hacker_group\n\n",
                        )
                else:
