...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Akki Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("akki_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="akki_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("« Bᴀᴄᴋ", data="ownr")],
            ],
        )
        remove("akki_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Akki Op", text=txt, link_preview=False, buttons=btn)
    ]
