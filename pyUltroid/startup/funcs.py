...
async def autobot():
    ...
    if who.username:
        ...
        username = "akki_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if not channel:
        ...
        try:
            r = await ultroid_bot(
                CreateChannelRequest(
                    title="My Akki Logs",
                    about="My Akki Log Group\n\n Join @tgr_hacker_group",
                    megagroup=True,
                ),
            )
            ...
            async for x in ultroid_bot.iter_messages(
                ...
                if not os.path.exists(plugin):
                    ...
                    try:
                        ...
                        LOGS.info(f"Akki - PLUGIN_CHANNEL - ERROR - {plugin}")
                        LOGS.exception(e)
                        os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
