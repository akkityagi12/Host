...
class MongoDB(_BaseDatabase):
    def __init__(self, key, dbname="AkkiDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        super().__init__()

    def __repr__(self):
        return f"<Akki.MonGoDB\n -total_keys: {len(self.keys())}\n>"
...
# for SQL Implementation in Akki.
#
...
class SqlDB(_BaseDatabase):
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS Akki (akkiCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('Akki')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'akki'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM Akki")
    ...
    def set(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE Akki DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE Akki ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO Akki ({key}) values (%s)", (str(value),))
        return True

    def delete(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE Akki DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE Akki")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS Akki (akkiCli varchar(70))"
        )
        return True
