...
from ..version import akki_version
...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @Tgr_hacker"
            await eod(ok, f"✓ `Akki - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("akki-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="akki-heroku.log",
            thumb=ULTConfig.thumb,
            caption="**Akki Heroku Logs.**",
        )

        os.remove("akki-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Akki {akki_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Akki {akki_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
